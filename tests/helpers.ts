...
// result so the suites assert on status and stderr alike. `env` is the
// script's WHOLE environment, not an overlay: a suite that wants the
// runner's variables spreads `process.env` in itself.
